#!/usr/bin/env python3
"""
Analyse COVID-19 Espagne - Première Vague
...
def load_spain_data_github():
    """Télécharge les données COVID-19 Espagne depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Espagne depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Espagne - somme de toutes les régions
        spain_data = df[df['Country/Region'] == 'Spain'].iloc[:, 4:].sum(axis=0)
        spain = pd.DataFrame({'deaths': spain_data})
        spain.index = pd.to_datetime(spain.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        spain = spain.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = spain['deaths'].diff().fillna(0)
        daily_deaths = daily_deaths.clip(lower=0)  # Enlever valeurs négatives

        # Lissage sur 7 jours
        daily_deaths_smooth = daily_deaths.rolling(window=7, center=True).mean().fillna(method='bfill').fillna(method='ffill')
        ...
        print(f"✅ Données Espagne chargées: {len(t_data)} points")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Espagne Vague 1 - Comparaison Super-Radiant vs SIR',
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 ESPAGNE VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Espagne...")
    t_data, y_data, dates, max_deaths_raw = load_spain_data_github()
    print()
    ...
    if best_model and sir_model:
        improvement = rms_percent_sir / best_rms
        print(f"📊 RÉSULTATS ESPAGNE VAGUE 1:")
    ...
    print()
    print("✅ Formule sech² validée sur les données espagnoles!")
    print("=" * 70)
