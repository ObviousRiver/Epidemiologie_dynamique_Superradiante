#!/usr/bin/env python3
"""
Analyse COVID-19 Irlande - Première Vague
...
def load_ireland_data_github():
    """Télécharge les données COVID-19 Ireland depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Ireland depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Irlande - somme de toutes les régions
        ireland_data = df[df['Country/Region'] == 'Ireland'].iloc[:, 4:].sum(axis=0)
        ireland = pd.DataFrame({'deaths': ireland_data})
        ireland.index = pd.to_datetime(ireland.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        ireland = ireland.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = ireland['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Ireland chargées: {len(t_data)} points")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Irlande Vague 1 - Comparaison Super-Radiant vs SIR',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Dublin)', 'Mode 2 (Munster)', 'Mode 3 (Connacht)', 'Mode 4 (Ulster)']
    ...
    ax3.set_title('Décomposition en 4 modes régionaux', fontsize=12, fontweight='bold')
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 ROYAUME-UNI VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Ireland...")
    t_data, y_data, dates, max_deaths_raw = load_ireland_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS ROYAUME-UNI VAGUE 1:")
    ...
    print()
    print("✅ Analyse Ireland terminée!")
    print("=" * 70)
