...
def load_portugal_data_github():
    """Télécharge les données COVID-19 Portugal depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Portugal depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Portugal - somme de toutes les régions
        portugal_data = df[df['Country/Region'] == 'Portugal'].iloc[:, 4:].sum(axis=0)
        portugal = pd.DataFrame({'deaths': portugal_data})
        portugal.index = pd.to_datetime(portugal.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        portugal = portugal.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = portugal['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Portugal chargées: {len(t_data)} points")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Portugal Vague 1 - Comparaison Super-Radiant vs SIR\n(Confinement STRICT - Test vs Suède)',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Lisbonne)', 'Mode 2 (Porto)', 'Mode 3 (Régional)', 'Mode 4 (Algarve)']
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 PORTUGAL VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("  TEST CRITIQUE: Portugal (strict) vs Suède (volontaire)")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Portugal...")
    t_data, y_data, dates, max_deaths_raw = load_portugal_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS PORTUGAL VAGUE 1:")
    ...
    if improvement < 1.0:
        print("   │ Portugal    │ État urgence (19 mars)│ SR       │ RMS {:.2f}%  │".format(best_rms))
    else:
        print("   │ Portugal    │ État urgence (19 mars)│ SIR      │ RMS {:.2f}%  │".format(rms_percent_sir))
    print("   └────────────┴─────────────────┴──────────┴─────────────┘")
    print()
    if improvement > 1.0:
    ...
    print()
    print("✅ Analyse Portugal terminée!")
    print("=" * 70)
