...
def load_denmark_data_github():
    """Télécharge les données COVID-19 Danemark depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Danemark depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Danemark - somme de toutes les régions
        denmark_data = df[df['Country/Region'] == 'Denmark'].iloc[:, 4:].sum(axis=0)
        denmark = pd.DataFrame({'deaths': denmark_data})
        denmark.index = pd.to_datetime(denmark.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        denmark = denmark.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = denmark['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Danemark chargées: {len(t_data)} points")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Danemark Vague 1 - Comparaison Super-Radiant vs SIR\n(Confinement STRICT - Test vs Suède)',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Copenhague)', 'Mode 2 (Jutland)', 'Mode 3 (Régional)', 'Mode 4 (Îles)']
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 DANEMARK VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("  TEST CRITIQUE: Danemark (strict) vs Suède (volontaire)")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Danemark...")
    t_data, y_data, dates, max_deaths_raw = load_denmark_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS DANEMARK VAGUE 1:")
    ...
    if improvement < 1.0:
        print("   │ Danemark    │ Strict (11 mars)│ SR       │ RMS {:.2f}%  │".format(best_rms))
    else:
        print("   │ Danemark    │ Strict (11 mars)│ SIR      │ RMS {:.2f}%  │".format(rms_percent_sir))
    print("   └────────────┴─────────────────┴──────────┴─────────────┘")
    print()
    if improvement > 1.0:
    ...
    print()
    print("✅ Analyse Danemark terminée!")
    print("=" * 70)
