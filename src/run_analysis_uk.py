#!/usr/bin/env python3
"""
Analyse COVID-19 Royaume-Uni - Première Vague
...
def load_uk_data_github():
    """Télécharge les données COVID-19 UK depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données UK depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Royaume-Uni - somme de toutes les régions
        uk_data = df[df['Country/Region'] == 'United Kingdom'].iloc[:, 4:].sum(axis=0)
        uk = pd.DataFrame({'deaths': uk_data})
        uk.index = pd.to_datetime(uk.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        uk = uk.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = uk['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données UK chargées: {len(t_data)} points")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Royaume-Uni Vague 1 - Comparaison Super-Radiant vs SIR',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (London)', 'Mode 2 (Angleterre)', 'Mode 3 (Écosse/Galles)', 'Mode 4 (Isolé)']
...
def main():
    ...
    print("[1/5] Chargement des données UK...")
    t_data, y_data, dates, max_deaths_raw = load_uk_data_github()
    print()
    ...
    output_path = reports_dir / "comparison_uk_wave1.png"
    ...
    if best_model:
        ...
        mode_names = ['London      ', 'Angleterre  ', 'Écosse/Galles', 'Isolé       ']
