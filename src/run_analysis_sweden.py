#!/usr/bin/env python3
"""
Analyse COVID-19 Suède - Première Vague
Comparaison Super-Radiant (sech²) vs SIR Standard

CAS TEST CRITIQUE: Suède sans confinement strict
...
def load_sweden_data_github():
    """Télécharge les données COVID-19 Suède depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Suède depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Suède - somme de toutes les régions
        sweden_data = df[df['Country/Region'] == 'Sweden'].iloc[:, 4:].sum(axis=0)
        sweden = pd.DataFrame({'deaths': sweden_data})
        sweden.index = pd.to_datetime(sweden.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        sweden = sweden.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = sweden['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Suède chargées: {len(t_data)} points")
        ...
        print(f"   ⚠️  STRATÉGIE UNIQUE: Pas de confinement strict")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Suède Vague 1 - Comparaison Super-Radiant vs SIR\n(Stratégie SANS confinement strict)',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Stockholm)', 'Mode 2 (Grandes villes)', 'Mode 3 (Régional)', 'Mode 4 (Rural)']
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 SUÈDE VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("  CAS TEST: Stratégie SANS confinement strict")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Suède...")
    t_data, y_data, dates, max_deaths_raw = load_sweden_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS SUÈDE VAGUE 1:")
    ...
    print()
    print("🧪 TEST HYPOTHÈSE: Pas de confinement strict")
    if improvement < 1.0:
        print("   ✅ CONFIRMÉ: Sans confinement → propagation asynchrone → SR meilleur")
    else:
        print("   ⚠️  INFIRMÉ: Dynamique plus complexe que prévu")
    print()
    print("✅ Analyse Suède terminée!")
    print("=" * 70)
